...
def acc(inputs, target):
    ...
    matrix = confusion_matrix(temp_target.cpu().numpy().flatten().astype(int), temp_inputs.cpu().numpy().flatten().astype(int))
...
def mean_iou(inputs, target, classes = 2):
    ...
    for i in range(classes):
        intersection = torch.logical_and(temp_target == i, temp_inputs == i)
        # print(intersection.any())
        union = torch.logical_or(temp_target == i, temp_inputs == i)
        temp = torch.sum(intersection) / torch.sum(union)
        miou += temp
    return  miou/float(classes)
